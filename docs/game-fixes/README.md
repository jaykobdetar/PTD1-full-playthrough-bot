# Game fixes for the main builds

Updated 2026-09-09. The user authorized fixing the game before the final starter-and-captures recordings. This directory packages the production fixes separately from the bot and video implementation. Nothing has been committed, pushed, or applied to another build.

## Apply the bundle

- [game-fixes.patch](game-fixes.patch): seven production files and seven regression-test files, including two new standalone tests. No bot, video, dependency, or game-asset changes.
- [manifest.json](manifest.json): exact base commit, patch hash, before/after SHA-256 for every file, and verified application result.
- Base: `2f73d0c8588e52a8c0c53ee1485a289c34e2962b` in this Phaser-PTD1 repository.

On a clean branch in the target build, copy this directory somewhere accessible, then use the actual path to the patch:

```sh
git apply --check /path/to/game-fixes.patch
git apply /path/to/game-fixes.patch
npm ci
npm test
npm run build
```

If the target has diverged, review and port the affected methods in the table below. A failed `--check` means the patch has not been applied. Do not treat an application conflict as a reason to replace whole files with this checkout's versions. These fixes do not require installing the bot or its video dependencies.

The package script verifies application to isolated copies of the exact base files and compares every resulting file's hash. Regenerate it after a production/test edit with `node tools/package-game-fixes.mjs`. That verification establishes patch integrity, not compatibility with a different build. For an uncommitted, otherwise unchanged application, `git apply --reverse --check` followed by `git apply --reverse` can remove this patch; normal commits should be reverted through Git.
...
**Evidence limit:** the original defect was isolated through a quest-prerequisite fixture, not a claimed complete quest playthrough. See [historical QA-001](../BOT_BUG_FINDINGS.md#qa-001--rock-tunnels-model-reward-misses-the-immediate-pokédex-update).
...
The recovered ActionScript was read from the user's other local source tree. Its path and the earlier missing-source conclusion are documented in [BOT_GAME_FIXES.md](../BOT_GAME_FIXES.md#original-reward-rule-evidence). This package does not overwrite or modify that source tree.
...
**Recorded failure:** `strict-candidate-1/auxiliary-1-1/checkpoints/01164.json`, seed 1860322713, stage 31, frame 15393. The battle won but the resulting profile could not be saved. The checkpoint, failing attempt and stack are archived with the other [combat/acquisition failures](../bot-repros/combat-crashes/README.md).
...
## Verification status and unresolved work

- Full suite: **338/338 passed**, including both QA-009 capture boundaries. The production build passed with its existing large-chunk warning. [Verification logs and results](verification/results.json) preserve the commands, output and hashes. This suite includes bot tests; target builds should run their own full suite after applying the production-only bundle. The packaged patch has also been applied and hash-checked in isolation.
- Patched strategy diagnostic: 42/42 campaign entries completed without recorded runtime errors. A subsequent continuation cleared all six challenges. These are resumed diagnostics, not a single fresh deterministic completion proof.
- The strict diagnostics have earned all three legendary bird rewards and caught Mew/Mewtwo after training the catcher to level 90. An actual continuation also earned Aerodactyl at stage 32 (seed 814657751, 7800 frames, all eight candies retained, `dexGained: [142]`). These diagnostics collectively establish earned acquisition routes for all 151 species, but are not one fresh complete run. The first fresh strict run subsequently stopped at 149/151 because of two bot selection defects. Correcting those defects and resuming its actual save earned both missing species in two visits, reaching 151/151. [The bot defect reproductions and recovery](../bot-repros/strict-collection-blockers/README.md) are separate from this production patch. A new independent fresh run is still being validated. The final whole-game videos are not yet complete. The separate four-level patched smoke produced 1×, 20× and 60× videos with verified frame counts/durations.
- No browser-input, audio, network-persistence or existing-save migration coverage is claimed by the headless run. The video layer renders a deterministic replay; menu activity cards are explicitly reconstructed from action logs.

## Problems that are not game fixes

These remain in bot strategy/run evidence, not in `game-fixes.patch`:

| Observation | Classification and response |
| --- | --- |
| Stage 25 defeats attackers with Destiny Bond | Native rule: attackers at/below the enemy's level can be defeated. Train the team through earned XP; do not change Destiny Bond. |
| Earthquake defeats allied towers | Native friendly fire. Reduce the bot's preference for it; do not remove the game's ally damage. |
| Low-level restored fossils were selected for difficult battles | Party-selection weakness. Develop them through real training and avoid choosing them as primary counters too early. |
| A training map has no legal trainee/support pair | Harness planning failure. Select compatible geometry; never relax deployment rules. |
| Rare Mew/Mewtwo repeatedly escape with most HP intact | Capture strategy needs a stronger trained catcher and appropriate moves. Encounter probability is not evidence of a bug. |
| An auxiliary keeps replaying a later stage while an earlier Safari target is missing | Orchestration weakness. Return to unlocked capture routes once the required routes are ready, and stop when requested species are earned. |
| Moltres remains on the map after all attackers faint | The recovered original stage also lacks a party-defeat exit here. This is a source-behavior/usability issue, not a demonstrated translation regression. The bot uses a tick limit and retries; earned Rest/Body Slam moves enabled a real win. |
| A run exhausts retries, battle visits, or collection visits | Incomplete strategy/budget, not a game defect or a synthetic victory. |
| False Swipe appears off by one in its early attack body | Ruled out: the delayed impact handler correctly preserves 1 HP on exactly lethal damage. |
| Save reload adds nickname/tag defaults or normalizes `haveFlash` | Benign normalization; no lost progression was demonstrated. |

The five combat/acquisition failure checkpoints and their stacks are also archived under [combat-crashes](../bot-repros/combat-crashes/README.md), so the evidence is retained outside the ignored artifact directory.

See [BOT_BUG_FINDINGS.md](../BOT_BUG_FINDINGS.md) for the historical failures, [BOT.md](../BOT.md) for run rules, and each run's `BUGS.md`, `report.json`, `actions.jsonl`, `events.jsonl` and checkpoint directory for exact replay context. Historical recordings/adoption-assisted runs are not proof of starter-and-captures completion.
